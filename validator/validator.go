...
var (
	...
	jsonContent  = regexp.MustCompile(`(?i)application/((\w|\.|-)+\+)?json(-seq)?`)
)
...
func (v *Validator) Validate() url.Values {
	...
	dataFace, err := FromRequest(v.Opts.Request)
	if err != nil {
		errsBag.Add("panic", err.Error())
		return errsBag
	}
		...
		for _, rule := range rules {
			...
			if strings.HasPrefix(field, "file:") {
				...
				if file != nil && fh.Filename != "" {
					...
					validateCustomRules(fld, rule, msg, file, dataFace, errsBag)
				} else {
					validateCustomRules(fld, rule, msg, nil, dataFace, errsBag)
				}
			} else {
				// validate if custom rules exist

				var reqVal interface{}
				v, has := dataFace.Get(field)
				if has {
					reqVal = v
				}
				// reqVal := strings.TrimSpace(dataFace.Get(field))
				validateCustomRules(field, rule, msg, reqVal, dataFace, errsBag)
			}
		}
	}
...
func FromJSONBytes(bs []byte) (*MapDataSource, error) {
...
func (v *Validator) internalValidateStruct() url.Values {
	...
	for field, rules := range v.Opts.Rules {
		...
		for _, rule := range rules {
			...
			msg := v.getCustomMessage(field, rule)
			validateCustomRules(field, rule, msg, value, nil, errsBag)
		}
	}
